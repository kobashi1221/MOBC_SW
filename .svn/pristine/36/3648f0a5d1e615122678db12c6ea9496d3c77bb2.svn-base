...
long iday, iyear;
    ...
    iyear  = (long)orb->ep_year;//2022 右矢印　122
    ...
    if (iyear < 1901 || iyear > 2099)
        {
        printf("init_sgdp4: Satellite ep_year error %ld", iyear);
        imode = SGDP4_ERROR;
        return imode;
        }
...
//    printf("SGP4 rev %lf %lf\r\n", eo, xno);
    ...
    if (eo < (real)0.0 || eo > ECC_LIMIT_HIGH)
        {
        printf("init_sgdp4: Eccentricity out of range for %ld (%le)", Isat, (double)eo);
        imode = SGDP4_ERROR;
        return imode;
        }
    ...
    if (xno < 0.035*TWOPI/XMNPDA || xno > 18.0*TWOPI/XMNPDA)
        {
        printf("init_sgdp4: Mean motion out of range %ld (%le)", Isat, xno);
        imode = SGDP4_ERROR;
        return imode;
        }
    ...
    if (xincl < (real)0.0 || xincl > (real)PI)
        {
        printf("init_sgdp4: Equatorial inclination out of range %ld (%le)", Isat, DEG(xincl));
        imode = SGDP4_ERROR;
        return imode;
        }
    ...
    */
    ...
    if (imode == SGDP4_NEAR_NORM)
        ...
        printf("init_sgdp4: Deep space equations not supported");
#else
...
#define ts tsince
	...
	xmp   = (double)xmo + xmdot * tsince;//xmdf
	xnode = xnodeo + ts * (xnodot + ts * xnodcf);//nodem
	...
	switch(imode)
		...
		default:
			...
			printf("sgdp4: Orbit not initialised");
			return SGDP4_ERROR;
		}
