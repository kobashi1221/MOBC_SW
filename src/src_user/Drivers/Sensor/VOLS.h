...
#ifndef C2A_DRIVERS_ACS_VOLS_H_
#define C2A_DRIVERS_ACS_VOLS_H_
#ifdef TWO_U
//#include <main.h>
...
class VOLS_CLASS :  ADC_CLASS
{
	private:
		...
		//-----------------------------------------
		int VOLS_SENSOR_AD7654_Get(int state);
	...
	public:
		...
		float volmtqa;
		float volmtqo;
		uint16_t ADC_volmtqa;
		uint16_t ADC_volmtqo;
...
//		uint16_t ADC_intData;
		...
		// 衛星ソフトウェア依存関数宣言
		//-----------------------------------------
		int VOLS_SENSOR_init();
		int VOLS_SENSOR_get_all();
		int VOLS_SENSOR_get_MTQA();
		int VOLS_SENSOR_get_MTQO();
